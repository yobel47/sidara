...
<div class="flex flex-col min-h-full">
    ...
    {{-- ══════════════════════════════════════
         HB STATUS CARD (mobile only)
    ══════════════════════════════════════ --}}
    @if($pregnancyStatus !== 'tidak hamil' && $lastAncVisit !== null)
    @php
    $hbBadgeColor = [
    ...
    ][$lastAncVisit['diagWarna']] ?? 'bg-gray-100 text-gray-600';

    $daysAgoText = match(true) {
    $lastAncVisit['daysAgo'] === 0 => 'hari ini',
    $lastAncVisit['daysAgo'] < 7=> $lastAncVisit['daysAgo'] . ' hari lalu',
        $lastAncVisit['daysAgo'] < 30=> intdiv($lastAncVisit['daysAgo'], 7) . ' minggu lalu',
            default => intdiv($lastAncVisit['daysAgo'], 30) . ' bulan lalu',
            };
            @endphp
            <div class="lg:hidden bg-pink-50 px-5 pb-3">
                <div class="bg-white rounded-2xl border border-rose-100 shadow-sm px-5 py-4">
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Hemoglobin Terakhir</p>
                        <p class="text-[10px] text-gray-400">Kunjungan ke-{{ $lastAncVisit['visitNumber'] }}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="flex items-baseline gap-1">
                                <span
                                    class="text-3xl font-extrabold text-gray-900">{{ $lastAncVisit['hemoglobin'] }}</span>
                                <span class="text-sm font-medium text-gray-400">g/dL</span>
                            </div>
                            <p class="text-xs text-gray-400 mt-0.5">{{ $daysAgoText }}</p>
                        </div>
                        <div class="flex flex-col items-end gap-2">
                            <span class="text-xs font-bold px-3 py-1.5 rounded-full {{ $hbBadgeColor }}">
                                {{ $lastAncVisit['diagKategori'] }}
                            </span>
    ...
    ══════════════════════════════════════ --}}
            <div class="bg-pink-50 px-4 pt-5 pb-8 lg:px-10 lg:pt-0 lg:pb-0">
                ...
                @if($pregnancyStatus !== 'tidak hamil')
                {{-- ── HAMIL / MELAHIRKAN: semua 5 menu ── --}}
                ...
                <div class="grid grid-cols-3 lg:grid-cols-5 gap-3 mb-3 lg:mb-0">
                    ...
                    <a href="{{ route('skrining-awal') }}" wire:navigate class="bg-white rounded-2xl p-3 lg:p-5 flex flex-col items-center text-center
                      ...
                      transition-all duration-150">
                        ...
                        <span class="text-[9px] lg:text-xs text-gray-400 leading-relaxed">Pemeriksaan awal untuk
                            mengetahui
                            risiko anemia.</span>
                    </a>
                    ...
                    <a href="{{ route('persalinan') }}" wire:navigate class="bg-white rounded-2xl p-3 lg:p-5 flex flex-col items-center text-center
                      ...
                      transition-all duration-150">
                        ...
                        <span class="text-[9px] lg:text-xs text-gray-400 leading-relaxed">Catat data persalinan dan
                            kondisi ibu
                    ...
                    <a href="{{ route('data-bayi') }}" wire:navigate class="hidden lg:flex flex-col items-center text-center bg-white rounded-2xl p-5
                      shadow-sm border border-pink-50 hover:shadow-md transition-all duration-150">
                        ...
                        <span class="text-xs text-gray-400 leading-relaxed">Catat identitas dan data bayi baru lahir
                            secara
                            lengkap.</span>
                    </a>
                      ...
                      shadow-sm border border-pink-50 hover:shadow-md transition-all duration-150">
                        <div class="w-16 h-16 rounded-2xl bg-teal-50 flex items-center justify-center mb-3">
                            <svg viewBox="0 0 64 64" class="w-12 h-12" fill="none">
                                ...
                                <path d="M32 25 V35 M27 30 H37" stroke="#0d9488" stroke-width="2.5"
                                    stroke-linecap="round" />
                                <line x1="20" y1="43" x2="44" y2="43" stroke="#0d9488" stroke-width="2"
                                    stroke-linecap="round" opacity="0.6" />
                                <line x1="20" y1="49" x2="38" y2="49" stroke="#0d9488" stroke-width="2"
                                    stroke-linecap="round" opacity="0.4" />
                            </svg>
                        </div>
                        <span class="text-sm font-bold text-gray-800 mb-1">Rekam Medis</span>
                        <span class="text-xs text-gray-400 leading-relaxed">Lihat ringkasan seluruh rekam medis dari
                            awal hingga
                            data bayi.</span>
                    </a>
                ...
                <div class="grid grid-cols-2 gap-3 mt-3 lg:hidden">
                    ...
                    <a href="{{ route('data-bayi') }}" wire:navigate class="bg-white rounded-2xl p-4 flex flex-col items-center text-center
                      shadow-sm border border-pink-50 active:scale-95 transition-all duration-150">
                        ...
                        <span class="text-[9px] text-gray-400 leading-relaxed">Catat identitas dan data bayi baru lahir
                            secara
                            lengkap.</span>
                    </a>
                    ...
                    <a href="{{ route('rekam-medis') }}" wire:navigate class="bg-white rounded-2xl p-4 flex flex-col items-center text-center
                      shadow-sm border border-pink-50 active:scale-95 transition-all duration-150">
                        ...
                        <span class="text-[9px] text-gray-400 leading-relaxed">Lihat ringkasan seluruh rekam medis dari
                            awal
                ...
                @else
                {{-- ── TIDAK HAMIL: hanya 2 menu aktif ── --}}

                <div class="grid grid-cols-2 gap-3">

                    {{-- Skrining Awal --}}
                    <a href="{{ route('skrining-awal') }}" wire:navigate class="bg-white rounded-2xl p-4 flex flex-col items-center text-center
                      shadow-sm border border-pink-50 active:scale-95 lg:hover:shadow-md transition-all duration-150">
                        <div class="w-16 h-16 rounded-2xl bg-rose-50 flex items-center justify-center mb-3">
                            <svg viewBox="0 0 56 56" class="w-12 h-12" fill="none">
                                <path
                                    d="M24 8 C24 8 14 20 14 28 C14 34.627 18.477 40 24 40 C29.523 40 34 34.627 34 28 C34 20 24 8 24 8Z"
                                    fill="#fda4af" />
                                <path
                        ...
                        <span class="text-xs font-bold text-gray-800 leading-tight mb-1">Skrining Awal</span>
                        <span class="text-[9px] text-gray-400 leading-relaxed">Pemeriksaan awal untuk mengetahui risiko
                            anemia.</span>
                    </a>
                    ...
                    <a href="{{ route('rekam-medis') }}" wire:navigate class="bg-white rounded-2xl p-4 flex flex-col items-center text-center
                      shadow-sm border border-pink-50 active:scale-95 lg:hover:shadow-md transition-all duration-150">
                        ...
                        <span class="text-xs font-bold text-gray-800 leading-tight mb-1">Rekam Medis</span>
                        <span class="text-[9px] text-gray-400 leading-relaxed">Lihat ringkasan seluruh rekam medis dari
                            awal hingga data bayi.</span>
                    </a>

                </div>

                {{-- Card perbarui status --}}
                <div class="mt-5 bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl border border-rose-100 p-4">
                    <div class="flex items-start gap-3">
                        <div class="w-9 h-9 rounded-xl bg-rose-100 flex items-center justify-center shrink-0 mt-0.5">
                            <svg class="w-4.5 h-4.5 text-rose-500" style="width:18px;height:18px" fill="none"
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-extrabold text-rose-700 mb-0.5">Sudah hamil?</p>
                            <p class="text-xs text-rose-400 leading-relaxed lg:mb-0 mb-3">
                                Perbarui status kehamilanmu untuk mengakses fitur Kehamilan, Persalinan, dan Data Bayi.
                            </p>
                            <a href="{{ route('perbarui-status') }}" wire:navigate class="lg:hidden inline-flex items-center gap-1.5 px-4 py-2 rounded-xl
                                       bg-rose-500 hover:bg-rose-600 active:scale-95
                                       text-white text-xs font-bold tracking-wide transition-all
                        ...
                        <div class="min-w-0 h-full hidden lg:flex" style="align-self: center;">
                            <a href="{{ route('perbarui-status') }}" wire:navigate class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl
                                       bg-rose-500 hover:bg-rose-600 active:scale-95
                                       text-white text-xs font-bold tracking-wide transition-all
                                       shadow-sm shadow-rose-200">
                                Perbarui Status
